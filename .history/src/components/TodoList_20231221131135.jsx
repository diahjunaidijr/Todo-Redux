  ...
  const handleAddTodo = () => {
    if (inputValue.trim() !== '') {
      dispatch(addTodoAsync(inputValue))
        .then(() => {
          setInputValue('');
          setUpdateTime(new Date()); // Set the creation time
        })
        .catch((error) => {
  ...
  const handleSaveEdit = () => {
    dispatch(editTodoAsync({ id: editTodoId, task: editedText }))
      .then(() => {
        setEditTodoId(null);
        setEditedText('');
        setUpdateTime(new Date()); // Set the update time
      })
      .catch((error) => {
              ...
              {editTodoId === todo.id ? (
                ...
                <>
                  ...
                  <span className="text-sm text-gray-400">
                    {updateTime ? 'UpdatedAt: ' : 'CreatedAt: '}
                    {new Intl.DateTimeFormat('en-GB', {
                      dateStyle: 'full',
                      timeStyle: 'long',
                      timeZone: 'Asia/Jakarta',
                    }).format(updateTime || new Date(todo.createdAt))}
                  </span>
