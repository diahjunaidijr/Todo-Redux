...
const TodoList = () => {
  ...
  useEffect(() => {
    ...
    const storedUpdateTime = localStorage.getItem('updateTime');
    const storedCreateTime = localStorage.getItem('createTime');
    const updateTime = storedUpdateTime ? JSON.parse(storedUpdateTime) : {};
    const createTime = storedCreateTime ? new Date(JSON.parse(storedCreateTime)) : null;
      ...
      dispatch(addTodoAsync(inputValue))
        .then(() => {
          ...
          if (isEditing) {
            ...
            const newCreateTime = new Date();
            setCreateTime(newCreateTime);
            localStorage.setItem('createTime', JSON.stringify(newCreateTime));
          }
        })
        .catch((error) => {
